    ...
    create: function (context) {
        ...
        function shouldCheck(node) {
            ...
            if (node && node.type === 'CallExpression' && node.callee.type === 'MemberExpression') {
                ...
                const isEach = node.callee.property.type === 'Identifier' && functionNames.includes(node.callee.property.name)
                if (isLod && isEach) {
                    let CallBackNode = node.arguments[1];   //获取第二个参数
                    const isArrowFunction = CallBackNode && CallBackNode.type === 'ArrowFunctionExpression';
                    const isFunction = CallBackNode && CallBackNode.type === 'FunctionExpression';
                    if (isArrowFunction) {
